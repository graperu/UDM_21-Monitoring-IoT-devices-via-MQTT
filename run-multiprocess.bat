...
for %%D in (temp_hum_01 air_quality_01 power_meter_01 smart_light_01 door_sensor_01) do (
    start "IoT %%D" cmd /k "dotnet run --no-build -c Release --project Code/Simulators/Simulators.csproj -- --host %UDM21_BROKER% --port %UDM21_PORT% --topic-root %UDM21_ROOT% --device %%D"
)

ping 127.0.0.1 -n 3 >nul
start "WPF Dashboard" cmd /k "dotnet run --no-build -c Release --project Code/Dashboard/Dashboard.csproj"
